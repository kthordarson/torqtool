...
class TripPlotWorker(QObject):
    ...
    def run(self):
        ...
        try:
            ...
            preview_latlon_by_file: dict[int, tuple[list[float], list[float]]] = {}
            ...
            for current_index, fileid in enumerate(self.fileids, start=1):
                ...
                lat_list = lat_vals.tolist()
                lon_list = lon_vals.tolist()
                preview_latlon_by_file[int(fileid)] = (lat_list, lon_list)
                all_lat.extend(lat_list)
                all_lon.extend(lon_list)
                self.preview.emit(
                    self.request_id,
                    {
                        "done": current_index,
            ...
            for current_index, fileid in enumerate(self.fileids, start=1):
                ...
                point_count = min(len(lat_list), len(lon_list), len(speed_vals))
                if point_count <= 0:
                    self.progress.emit(self.request_id, current_index, len(self.fileids))
                    continue
