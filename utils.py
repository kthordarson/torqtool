...
def get_parser(appname):
    ...
    parser.add_argument(
        "--fixer",
        default=False,
        help="run fixer, set --bakpath",
        action="store_true",
        dest="fixer",
    )
    parser.add_argument(
        "--fixcsv", default=False, help="repair csv", action="store_true", dest="fixcsv"
    )
    ...
    parser.add_argument(
        "--repairsplit",
        default=False,
        help="enable splitting of strange log files",
        action="store_true",
        dest="repairsplit",
    )
    parser.add_argument(
        "--samplemode",
        default=False,
        help="use samplemode, select small random number of logs-for debugging",
        action="store_true",
        dest="samplemode",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--testnewreader",
        default=False,
        help="run testnewreader",
        action="store_true",
        dest="testnewreader",
    )
    parser.add_argument(
        "--threadmode", default="ppe", help="threadmode ppe/oldppe/tpe", action="store"
    )
    parser.add_argument(
        "--torqdata",
        default=False,
        help="create torqdata",
        action="store_true",
        dest="torqdata",
    )
    parser.add_argument(
        "--transfer",
        default=False,
        help="transfer old logs, set oldlogpath to location of old triplogs",
        action="store_true",
        dest="transfer",
    )
    parser.add_argument(
        "--bakpath",
        nargs="?",
        default="/home/kth/development/torq/backups2",
        help="where to put backups",
        action="store",
    )
    parser.add_argument(
        "--check-file",
    ...
    parser.add_argument(
        "--chunks", nargs="?", default="4", help="chunks", action="store"
    )
    parser.add_argument(
        "--combinecsv",
        default=False,
    ...
    parser.add_argument(
        "--dbfile", default="torqfiskur.db", help="database file", action="store"
    )
    parser.add_argument(
        "--db_limit", default=False, help="db_limit", action="store", dest="db_limit"
    )
    parser.add_argument(
        "--dump-db",
        nargs="?",
    ...
    parser.add_argument(
        "--file", nargs="?", default=".", help="path to single csv file", action="store"
    )
    parser.add_argument(
        "--logpath", nargs="?", default=".", help="path to csv files", action="store"
    )
    parser.add_argument(
        "--max_workers", nargs="?", default="4", help="max_workers", action="store"
    )
    parser.add_argument(
        "--oldlogpath", nargs="?", default=".", help="oldlogpath", action="store"
    )
    parser.add_argument(
        "--sqlchunksize", nargs="?", default="1000", help="sql chunk", action="store"
    )
    parser.add_argument(
        "--webstart",
        default=False,
        help="start web listener",
        action="store_true",
        dest="web",
    )
    parser.add_argument(
...
def get_sanatized_column_names(orgcol):
    ...
    if isinstance(orgcol, list):
        newcolname = (
            ",".join([re.sub(r"\W", "", col).lower() for col in orgcol])
            .encode("ascii", "ignore")
            .decode()
        )
        newcolname += "\n"
        ...
        newcolname = (
            ",".join([re.sub(r"\W", "", col).lower() for col in orgcol.split(",")])
            .encode("ascii", "ignore")
            .decode()
        )
        newcolname += "\n"
    ...
    try:
        ...
        if splits > 1:
            ...
            if Path(backupfile).exists():
                ...
                logger.warning(
                    f"backupfile {backupfile} exists, backing up to {newbakname} "
                )
...
def get_csv_files(searchpath: str, args):
    ...
    torqcsvfiles = [
        ...
        for k in Path(searchpath).glob("**/*.csv")
        if k.stat().st_size >= MIN_FILESIZE
    ]  # and not os.path.exists(f'{k}.fixed.csv')]
    return torqcsvfiles
    ...
    for line in data:
        ...
        for lx in l0:
            try:
                ...
                logger.error(
                    f"AttributeError: {e} in {csvfile} lt={type(line)} l={line}"
                )
...
def get_engine_session(args):
    ...
    if args.dbmode == "mysql":
        ...
        engine = create_engine(
            dburl, echo=False, connect_args={"check_same_thread": False}
        )
        # Session = sessionmaker(bind=engine)
        # session = Session()
    if not engine:
...
def sqlsender(buffer, dburl, debug=False):
    ...
    try:
        ...
        if e.code == "e3q8" and "Unknown column" in e.args[0]:
            ...
            logger.warning(
                f'[tosql] {newcol=} code={e} args={e.args} r={results} csvfile={buffer["csvfile"]}'
            )  # error:{e}
            ...
            logger.error(
                f'[tosql] code={e} r={results} csvfile={buffer["csvfile"]}'
            )  # error:{e}
            results["status"] = "error"
    except InternalError as e:
        logger.error(
            f'[tosql] InternalError {e} r={results} csvfile={buffer["csvfile"]}'
        )
        results["status"] = "error"
    except IntegrityError as e:
        logger.warning(
            f'[tosql] {type(e)} code={e} args={e.args[0]} r={results} csvfile={buffer["csvfile"]}'
        )
        results["status"] = "error"
        ...
        logger.warning(
            f'\n[tosql] {type(e)} code={e} err_row: {err_row} err_col:{err_col} torqfile={csvfile} fileid:{buffer["fileid"]}'
        )  # error:{e}
        ...
        try:
            ...
            logger.error(
                f'[torql] {type(exc)} {exc} err_row: {err_row} err_col:{err_col} torqfile={csvfile} fileid:{buffer["fileid"]}'
            )
        try:
            tmpbuf.to_sql("torqlogs", con=engine, if_exists="append", index=False)
...
def sqlsender_ppe(buffer, session, debug=False):
    ...
    try:
        tmpbuf.to_sql(
            "torqlogs", con=session.get_bind(), if_exists="append", index=False
        )
        results["status"] = "success"
        torqfile = (
            session.query(TorqFile).filter(TorqFile.fileid == results["fileid"]).first()
...
def read_buff(csvfile, tf_fileid, debug=False):
    ...
    try:
        torqbuffer = read_csv_polars(
            csvfile,
            ignore_errors=True,
            try_parse_dates=True,
            truncate_ragged_lines=True,
        )  # , use_pyarrow=True ,  ) #, null_values=['NaN','-','0\x88\x9e'])
...
def fix_timestamps(torqbuffer, csvfile, tf_fileid):
    ...
    try:
        if len(torqbuffer["devicetime"][idx]) == 28:
            devicetime = pl.Series(
                "devicetime",
                [
                    datetime.strptime(k, fmt_28).astimezone(pytz.timezone("UTC"))
                    for k in torqbuffer["devicetime"]
                    if k
                ],
            )
            ...
            devicetime = pl.Series(
                "devicetime",
                [
                    datetime.strptime(k, fmt_24).astimezone(pytz.timezone("UTC"))
                    for k in torqbuffer["devicetime"]
                    if k
                ],
            )
            ...
            devicetime = pl.Series(
                "devicetime",
                [
                    datetime.strptime(k, fmt_26).astimezone(pytz.timezone("UTC"))
                    for k in torqbuffer["devicetime"]
                    if k
                ],
            )
            ...
            devicetime = pl.Series(
                "devicetime",
                [
                    datetime.strptime(k, fmt_20).astimezone(pytz.timezone("UTC"))
                    for k in torqbuffer["devicetime"]
                    if k
                ],
            )
            ...
            logger.error(
                f'[rb] devicetime format error! len = {len(torqbuffer["devicetime"][idx])} {idx=} buffer: {torqbuffer["devicetime"]}'
            )
    except (ColumnNotFoundError, ComputeError, ValueError, TypeError) as e:
        logger.error(
            f'[rb] devicetime {type(e)} {e} csvfile: {csvfile} len = {len(torqbuffer["devicetime"][idx])} {idx=} '
        )
    ...
    try:
        if len(torqbuffer["gpstime"][idx]) == 28:
            ...
            gpstime = pl.Series(
                "gpstime",
                [
                    datetime.strptime(k, fmt_34).astimezone(pytz.timezone("UTC"))
                    for k in torqbuffer["gpstime"]
                    if k
                ],
            )
            ...
            logger.error(
                f'[rb] gpstime format error ex: {torqbuffer["gpstime"]} len: {len(torqbuffer["gpstime"])}'
            )
    except (ComputeError, ValueError, TypeError) as e:
        logger.error(
            f'[rb] {type(e)} {e} csvfile: {csvfile} len = {len(torqbuffer["devicetime"][idx])} {idx=} buf: {torqbuffer["gpstime"]}'
        )
        error_files.append(csvfile)
        # raise e
    ...
    try:
        ...
        logger.error(
            f"[rb] {type(e)} {e} csvfile: {csvfile} tblen={len(torqbuffer)} glen={len(gpstime)} dlen={len(devicetime)} {gpstime_err=} {devicetime_err=}"
        )
        error_files.append(csvfile)
    try:
...
async def torq_worker_ppe(tf, session, debug=False):
    ...
    try:
        ...
        if debug:
            logger.debug(
                f't: {(datetime.now()-t0).seconds}/{timetotal} fileid {results.get("fileid")} {results.get("status")} buffer: {len(buffer["torqbuffer"])}'
            )
    except (ValueError, TypeError, PicklingError) as e:
        logger.error(
            f'[!] {type(e)} {e} in sqlsender buffer.is_empty() {buffer["torqbuffer"].is_empty()}'
        )
        return None


def send_torqtripdata(
    stats_data: dict, session: sessionmaker, args: argparse.Namespace
):
    """
...
def check_database_columns(session, args=None, limit=1000):
    ...
    df = pd.DataFrame(
        session.execute(
            text(
                'select column_name from information_schema.columns where table_name = "torqlogs" order by table_name,ordinal_position'
            )
        ).all()
    )
    ...
    column_names = sorted(
        [k for k in set([k[0] for k in df.values]) if k not in skip_cols]
    )
    ...
    for col in column_names:
        ...
        if not limit:
            df = pd.DataFrame(
                session.execute(text(f"select {col} from torqlogs")).all()
            )
        else:
            df = pd.DataFrame(
                session.execute(text(f"select {col} from torqlogs limit {limit}")).all()
            )
        try:
        ...
        print(
            f"  {col:<{maxnlen}} nulls: {nulls:>3} nr: {nr:>3.3} {minval:>3.3} {mednval:>3.3} {meannval:>3.3} {maxnval:>3.3}"
        )
        # nullratio: {nullratio}
...
def get_tripfile_stats(fileid, session, args=None, limit=1000):
    ...
    logger.info(
        f"checking {fileid=} found {len(column_names)} columns in database, limit:{limit}"
    )
    maxnlen = max([len(k) for k in column_names])  # longest name, for formatting
    for col in column_names:
        if args.debug:
        ...
        if not limit:
            df = pd.DataFrame(
                session.execute(
                    text(f"select {col} from torqlogs where fileid={fileid}")
                ).all()
            )
        else:
            df = pd.DataFrame(
                session.execute(
                    text(
                        f"select {col} from torqlogs where fileid={fileid} limit {limit}"
                    )
                ).all()
            )
        try:
...
def generate_torqdata(
    df: pd.DataFrame, session: sessionmaker = None, args: argparse.Namespace = None
):
...
def convert_string_to_datetime(s: str):
    ...
    try:
        match fmt_selector:
            case 20:
                datetimeobject = datetime.strptime(s, fmt_20).astimezone(
                    pytz.timezone("UTC")
                )
            case 24:
                datetimeobject = datetime.strptime(s, fmt_24).astimezone(
                    pytz.timezone("UTC")
                )
            case 26:
                datetimeobject = datetime.strptime(s, fmt_26).astimezone(
                    pytz.timezone("UTC")
                )
            case 28:
                datetimeobject = datetime.strptime(s, fmt_28).astimezone(
                    pytz.timezone("UTC")
                )
            case 30:
                datetimeobject = datetime.strptime(s, fmt_30).astimezone(
                    pytz.timezone("UTC")
                )
            case 34:
                datetimeobject = datetime.strptime(s, fmt_34).astimezone(
                    pytz.timezone("UTC")
                )
            case 36:
                datetimeobject = datetime.strptime(s, fmt_36).astimezone(
                    pytz.timezone("UTC")
                )
            case _:
