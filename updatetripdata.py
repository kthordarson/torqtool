...
def collect_db_filestats(args, todatabase=True, droptable=True):
	...
	for fileidx, file in enumerate(file_ids.itertuples()):
		if args.extradebug:
			logger.debug(
				f"[{fileidx}/{len(file_ids)}] working on fileid {file.fileid} "
			)
		# results[file.fileid] = []
		total_rows = pd.DataFrame(
			session.execute(
				text(f"select count(*) from torqlogs where  fileid={file.fileid}")
			)
		).values[0][0]  # id>0 and
		if total_rows == 0:
		...
		for idx, column in enumerate(schema_datatypes):
			if args.extradebug:
				logger.debug(
					f"[{fileidx}/{len(file_ids)}] fileid {file.fileid}  col: {column} "
				)
			nulls = pd.DataFrame(
				session.execute(
					text(
						f"select count(*) as count from torqlogs where  fileid={file.fileid} and {column} is null "
					)
				).all()
			).values[0][0]  # id>0 and
			...
			if args.extradebug and nulls > 0:
				logger.debug(
					f"[{fileidx}/{len(file_ids)}/{idx}/{len(schema_datatypes)}] {file.fileid} - {column} nulls {nulls} ratio:  {nulls/total_rows} notnulls:{notnulls} ratio: {notnulls/total_rows}"
				)
...
def collect_db_columnstats(args):
	...
	total_rows = pd.DataFrame(
		session.execute(text("select count(*) from torqlogs"))
	).values[0][0]
	...
	for idx, column in enumerate(schema_datatypes):
		try:
			nulls = pd.DataFrame(
				session.execute(
					text(
						f"select count(*) as count from torqlogs where {column} is null"
					)
				).all()
			).values[0][0]
			notnulls = total_rows - nulls
			# dfval = df.values[0][0]
			logger.debug(
				f"[{idx}/{len(schema_datatypes)}-{len(results)}] {column} nulls {nulls} ratio:  {nulls/total_rows} notnulls:{notnulls} ratio: {notnulls/total_rows}"
			)
			results[column] = {
				"column": column,
				"nulls": nulls,
...
def collect_db_speeds(args):
	...
	logger.info(
		f"dbspeeds: dfres {df.to_sql(name='speeds', con=engine, if_exists='replace')}"
	)
	return 0
...
def collect_db_startends(args):
	...
	if not args.db_limit:
		df = pd.DataFrame(
			session.execute(
				text(
					"select fileid as fileid,min(latitude) as latmin,min(longitude) as lonmin,max(latitude) as latmax,max(longitude) as lonmax from torqlogs group by fileid;"
				)
			).all()
		).fillna(0)
	else:
		df = pd.DataFrame(
			session.execute(
				text(
					f"select fileid as fileid,min(latitude) as latmin,min(longitude) as lonmin,max(latitude) as latmax,max(longitude) as lonmax from torqlogs group by fileid limit {args.db_limit};"
				)
			).all()
		).fillna(0)
	...
	# res = session.execute(text('create table speeds as select fileid,avg(gpsspeedkmh) as speed,min(gpstime) as gpstime  from torqlogs group by fileid'))
	logger.info(
		f"dbstartends dfres: {df.to_sql(name='startends', con=engine, if_exists='replace')}"
	)
	return 0
