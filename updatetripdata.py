...
def collect_db_torqtrips(args):
	...
	batch_size = 300 if args.dbmode == "sqlite" else 1000
	inserted_rows = 0
	logger.debug(f"candidate fileids for torqtrips={len(fileids)}, processing in batches of {batch_size}")
	for batch_start in range(0, len(fileids), batch_size):
		...
		try:
			...
			logger.debug(f"Clearing existing torqtrips rows for batch {batch_start // batch_size + 1} placeholders: {len(placeholders)}, params: {len(params)}")
			session.execute(text(f"DELETE FROM torqtrips WHERE fileid IN ({placeholders})"), params)
			logger.info(f"Cleared existing torqtrips rows for batch {batch_start // batch_size + 1}")
			...
			logger.debug(f"Processing {len(rows)} aggregated rows for torqtrips batch {batch_start // batch_size + 1}")
			...
			session.rollback()
			logger.error(f"torqtrips batch {batch_start // batch_size + 1} failed: {e} ({type(e)})")
			return -1
