...
from torqtool_git.commonformats import fmt_20, fmt_24, fmt_26, fmt_28, fmt_30, fmt_34, fmt_36
from torqtool_git.datamodels import TorqFile, database_init
from torqtool_git.schemas import ncc
from torqtool_git.utils import get_engine_session, get_fixed_lines
...
def new_polars_csv_reader(logfile):
	...
	df = data.to_pandas()
	try:
		ncren = {k:ncc[k] for k in df.columns} # get columns to rename
...
def get_files_to_send(session:sessionmaker, args):
	...
	try:
		...
		if len(files_to_send) > 0:
			logger.info(f'found {len(files_to_send)} dbfiles:{len(unread_dbfiles)} files to process and send...')
		else:
			logger.warning(f'no files found in {args.path} ! dbfiles:{len(unread_dbfiles)} ...')
